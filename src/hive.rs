...
const HEXDIR: [HexCoord; 6] = [HexCoord(1, -1, 0), HexCoord(1, 0, -1), HexCoord(0, 1, -1), HexCoord(-1, 1, 0), HexCoord(-1, 0, 1), HexCoord(0, -1, 1)];
...
impl<T> HexBoard<T> {
	...
	fn all_top<'a>(&'a self) -> impl Iterator<Item = (&'a HexCoord, &'a T)> {
		self.map.iter().filter_map(|(x, v)| v.last().map(|p| (x, p)))
	}
	...
	fn perimeter(&self) -> HashSet<HexCoord> {
		...
		for &coord in self.map.keys() {
			if self.is_empty(coord) {continue}
	...
	fn reachable_without(&self, from: HexCoord, without: HexCoord) -> HashSet<HexCoord> {
		...
		while let Some(cur) = to_check.pop() {
			to_check.extend(self.neighbor_cells(cur).filter(|&c| c != without).filter(|&c| visited.insert(c)));
		}
	...
	fn passable_coords(&self, from: HexCoord) -> Vec<HexCoord> {
		...
		from.neighbors().filter(|c| !self.occupied().contains(c)).filter(|c| self.perimeter().contains(c)).filter(|c| c.neighbor_set().intersection(&from_neighbors).any(|&x| self.is_empty(x))).collect()
	}
...
impl<T: Into<char> + Clone + std::fmt::Debug> HexBoard<T> {
	fn disp(&self) -> String {
		...
		let map_offset: HashMap<(i16, i16), Option<T>> = self.map.clone().into_iter().map(|(k, v)| (k.to_offset(), v.last().cloned())).collect();
		...
		for r in min_r..=max_r {
			...
			for c in min_c..=max_c {
				res.push(map_offset.get(&(r, c)).cloned().flatten().map(|x| x.into()).unwrap_or('.'));
				res.push(' ');
			}
...
impl HivePiece {
	fn valid_dests(&self, board: &HexBoard<HivePiece>, coord: HexCoord) -> Vec<HexCoord> {
		...
		match self.bug {
			Queen => board.passable_coords(coord).into_iter().filter(|&c| board.neighbor_cells(c).count() > 1).collect(),
			Beetle => {
				let perimeter = board.perimeter();
				let occupied = board.occupied();
				// >1 check needed bc otherwise the beetle counts itself and wanders off
				coord.neighbors().filter(|c| perimeter.contains(c) || occupied.contains(c)).filter(|&c| board.neighbor_cells(c).count() > 1).collect()
			},
			Grasshopper => {
				let mut res = Vec::new();
				let perimeter = board.perimeter();
			...
			Spider => {
				let one_away: HashSet<HexCoord> = board.passable_coords(coord).into_iter().collect();
				let two_away: HashSet<HexCoord> = one_away.iter().flat_map(|&c| board.passable_coords(c)).filter(|&c| c != coord).collect();
				two_away.iter().flat_map(|&c| board.passable_coords(c)).filter(|&c| c != coord).filter(|c| !one_away.contains(c)).filter(|&c| board.neighbor_cells(c).count() > 1).collect()
			},
			Ant => {
				let mut passable: HashSet<HexCoord> = board.passable_coords(coord).into_iter().collect();
				...
				while let Some(c) = to_explore.pop() {
					...
					to_explore.extend(board.passable_coords(c).into_iter().filter(|&c| c != coord).filter(|&c| passable.insert(c)));
				}

				// gotta filter out moves to its own perimeter (stop ant from wandering off)
				passable.into_iter().filter(|&c| board.neighbor_cells(c).count() > 1).collect()
			},
		}
	}
}
...
impl HiveGame {
	pub fn new() -> Self {
		...
		let hand_w: HashMap<HiveBug, u8> = [(Queen, 1), (Beetle, 2), (Grasshopper, 3), (Spider, 2), (Ant, 3)].into_iter().collect();
	...
	fn valid_placements(&self) -> Vec<HiveMove> {
		...
		let pieces = if self.round == 3 && !queen_placed {
			vec![HivePiece{color: self.turn, bug: HiveBug::Queen}]
		} else {
			hand.keys().cloned().map(|b| HivePiece{color: self.turn, bug: b}).collect()
		};
		...
		let dests = if self.round == 0 {
			...
			self.board.perimeter().iter().cloned().filter(|&c| self.board.neighbor_pieces(c).all(|p| p.color == self.turn)).collect()
		};
	...
	pub fn disp(&self) -> String {
		format!("Round {}\nTurn {}\nBoard:\n{}", self.round, self.turn, self.board.disp())
	}
